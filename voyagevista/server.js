...
function renderTripPage(t) {
  ...
  const autoCarousel = (t.photos&&t.photos.length>0) ? `
  <section class="sec"><div class="sec-label">Photo Gallery</div>
  <div class="carousel-wrap">
    <div class="carousel-track" id="carTrack">
      ${[...t.photos,...t.photos].map((p,i)=>`<div class="car-slide"><img src="${p}" loading="lazy" onclick="openLB(${i % t.photos.length})"></div>`).join('')}
    </div>
  </div>
  <div class="swipe-dots">${t.photos.map((_,i)=>`<div class="dot${i===0?' active':''}" id="dot${i}"></div>`).join('')}</div>
  ...
  const resortHero=t.resortHeroPhoto?`<div class="resort-img"><img src="${t.resortHeroPhoto}"></div>`:'';

  const logistics=(t.flight||t.hotel)?`<section class="sec"><div class="sec-label">Booking Details</div>${t.hotel?`<div class="card"><div class="ci hotel-i">🏨</div><div class="cb"><div class="ct">${t.hotel}</div><div class="cs">${t.hotelAddr?t.hotelAddr+'<br>':''}${t.checkin||''}</div></div></div>${resortHero}`:''} ${t.flight?`<div class="card"><div class="ci flight-i">✈</div><div class="cb"><div class="ct">${t.flight}</div><div class="cs">${t.flightTime||''}${t.bookingRef?' · '+t.bookingRef:''}</div></div></div>`:''}</section><hr class="div">`:'';
  ...
  // ITINERARY + BOOKING BUTTONS SIDE BY SIDE
  const itin=(t.days&&t.days.some(d=>d.title||(d.events&&d.events.length)))?`<section class="sec"><div class="sec-label">Your Itinerary</div>${t.days.filter(d=>d.title||(d.events&&d.events.length)).map(d=>`<div class="day-block"><div class="day-lbl">${d.label}${d.date?' — '+d.date:''}</div>${d.title?`<div class="day-ttl">${d.title}</div>`:''} ${(d.events||[]).filter(e=>e.title).map(e=>`<div class="ev-row"><span class="ev-dot ev-${e.type}"></span><div><div class="ev-title">${e.title}${e.time?` <span class="ev-time">${e.time}</span>`:''}</div>${e.notes?`<div class="ev-note">${e.notes}</div>`:''}</div></div>`).join('')}</div>`).join('')}</section><hr class="div">`:'';

  const docButtons = (t.itineraryLink||t.bookingUrl) ? `
  <section class="sec">
    <div class="sec-label">Documents & Booking</div>
    <div class="doc-btns">
      ${t.itineraryLink?`<a href="${t.itineraryLink}" target="_blank" rel="noopener" class="doc-btn itinerary-btn">📄 ${t.itineraryLinkLabel||'Download Itinerary'}</a>`:''}
      ${t.bookingUrl?`<a href="${t.bookingUrl}" target="_blank" rel="noopener" class="doc-btn booking-url-btn">🔗 ${t.bookingUrlLabel||'Book Now'}</a>`:''}
    </div>
  ...
  const bookSec=t.showBookingForm?`<section class="sec"><div class="sec-label">Booking Form</div><div class="book-form"><form id="rsvpForm"><div class="frow"><input type="text" placeholder="Full Name *" required id="bn"><input type="email" placeholder="Email *" required id="be"></div><div class="frow"><input type="tel" placeholder="Phone Number" id="bp"><input type="text" placeholder="Number of Guests" id="bg"></div>${(t.bookingFormFields||[]).map(f=>`<input type="text" placeholder="${f}" style="width:100%;margin-bottom:12px;">`).join('')}<textarea placeholder="Special requests, dietary requirements, or any questions..." rows="3" id="bn2"></textarea><button type="submit" class="sub-btn">Submit Booking to Voyage Vista Travels ✈</button></form><div id="bsuccess" style="display:none;text-align:center;padding:24px;color:#7ec98f;line-height:1.7;">✅ Thank you! We'll be in touch at <strong style="color:#c4a057;">Hello@voyagevista.ca</strong> within 1–2 business days.</div></div></section><hr class="div">`:'';
...
.hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%,rgba(196,160,87,.18),transparent 65%);pointer-events:none;}
.hi{position:relative;z-index:1;max-width:700px;margin:0 auto;}
...
.conf{font-size:1.4rem;letter-spacing:4px;margin-bottom:14px;opacity:.85;}
...
.h-dest{font-size:1.1rem;color:rgba(255,255,255,.65);margin-bottom:12px;}
...
.stat-box{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:14px 22px;}
...
.cd-wrap{display:inline-block;background:linear-gradient(135deg,rgba(196,160,87,.2),rgba(232,200,122,.12));border:2px solid rgba(196,160,87,.5);border-radius:20px;padding:24px 48px;margin-top:8px;}
.cd-sp{font-size:1.4rem;letter-spacing:6px;margin-bottom:8px;animation:sp 2s ease-in-out infinite;}
@keyframes sp{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.7;transform:scale(1.05)}}
.cd-num{font-family:'Cormorant Garamond',serif;font-size:clamp(4rem,12vw,8rem);color:#e8c87a;font-weight:600;line-height:1;animation:cdp 3s ease-in-out infinite;}
@keyframes cdp{0%,100%{text-shadow:0 0 40px rgba(196,160,87,.5)}50%{text-shadow:0 0 80px rgba(232,200,122,.8)}}
.cd-lbl{font-size:.85rem;color:rgba(255,255,255,.7);margin-top:4px;letter-spacing:.06em;text-transform:uppercase;}
...
/* AUTO-MOVING CAROUSEL */
.carousel-wrap{width:100%;overflow:hidden;border-radius:18px;position:relative;background:#1a2e45;}
.carousel-track{display:flex;animation:autoScroll ${Math.max(8, (t.photos||[]).length * 4)}s linear infinite;}
.carousel-track:hover{animation-play-state:paused;}
.car-slide{flex-shrink:0;width:min(600px, 85vw);aspect-ratio:16/9;overflow:hidden;border-radius:12px;margin:8px 6px;cursor:pointer;}
.car-slide img{width:100%;height:100%;object-fit:cover;transition:transform .3s;}
.car-slide:hover img{transform:scale(1.03);}
@keyframes autoScroll{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
.swipe-dots{display:flex;justify-content:center;gap:6px;padding:10px 0 4px;}
.dot{width:8px;height:8px;border-radius:50%;background:rgba(196,160,87,.3);transition:all .2s;}
...
.resort-img{width:100%;border-radius:12px;overflow:hidden;margin:12px 0;aspect-ratio:16/7;}
...
/* DOC BUTTONS SIDE BY SIDE */
.doc-btns{display:flex;gap:12px;flex-wrap:wrap;}
.doc-btn{display:inline-flex;align-items:center;gap:8px;padding:14px 24px;border-radius:12px;font-weight:500;font-size:14px;text-decoration:none;transition:all .2s;font-family:'DM Sans',sans-serif;flex:1;justify-content:center;min-width:160px;}
.itinerary-btn{border:2px solid #c4a057;color:#c4a057;background:transparent;}
.itinerary-btn:hover{background:#c4a057;color:#0d1b2a;}
...
.booking-note{background:#faf7f2;border-left:3px solid rgba(196,160,87,.4);border-radius:0 8px 8px 0;padding:10px 14px;font-size:13px;color:#888;margin-top:12px;}
...
@media(max-width:480px){.frow{grid-template-columns:1fr;}.doc-btns{flex-direction:column;}}
...
.social-bar{display:flex;justify-content:center;gap:12px;padding:24px 20px 8px;}
.social-link{width:44px;height:44px;border-radius:12px;background:#0d1b2a;border:1px solid rgba(196,160,87,.25);display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.5);text-decoration:none;transition:all .2s;}
.social-link:hover{border-color:#c4a057;color:#c4a057;background:#162336;}
...
${autoCarousel}${resortBtn}${logistics}${msgSec}${itin}${docButtons}${weatherSec}${packSec}${currSec}${bookSec}${contactSec}
${socialLinks}
<footer class="footer"><strong>Voyage Vista Travels</strong> · Nepean, ON · (343) 961-3506 · Hello@voyagevista.ca<br>Affiliated with Nexion Travel Group-Canada · TICO Reg: 1549342</footer>
...
<script>
var photos=${JSON.stringify(t.photos||[])},lbIdx=0;
...
if(form)form.addEventListener('submit',function(e){e.preventDefault();fetch('/api/booking',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:document.getElementById('bn').value,email:document.getElementById('be').value,to:'Hello@voyagevista.ca',trip:'${t.id}'})});form.style.display='none';document.getElementById('bsuccess').style.display='block';});
<\/script>
</body></html>`;
}
