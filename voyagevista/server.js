...
app.get('/api/trips', async (req, res) => {
  try {
    ...
    console.error('List trips error:', e.message);
...
app.get('/api/trips/:slug', async (req, res) => {
  try {
    ...
    console.error('Get trip error:', e.message);
    res.status(500).json({ error: 'Failed to load trip' });
  }
});

// ── API: SAVE / UPDATE TRIP ───────────────────────────
app.post('/api/trips', async (req, res) => {
  try {
    const trip = req.body;
    if (!trip.guestName || !trip.occasion) {
    ...
    const { error } = await supabase
      ...
      .upsert({
        ...
        guest_count: trip.guestCount  || null,
    ...
    console.error('Save trip error:', e.message);
    res.status(500).json({ error: 'Failed to save trip: ' + e.message });
  }
});
...
app.delete('/api/trips/:slug', async (req, res) => {
  try {
    ...
    console.error('Delete trip error:', e.message);
  ...
  try {
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Voyage Vista Travels'
      },
      body: JSON.stringify({
        model: req.body.model || 'anthropic/claude-3.5-haiku',
        max_tokens: 400,
...
app.post('/api/booking', async (req, res) => {
  try {
    ...
    console.log('RSVP received:', booking);
    // Optionally save to Supabase bookings table in the future
    res.json({ success: true });
  } catch(e) {
...
app.get('/trips/:slug/resort', async (req, res) => {
  try {
    const { data, error } = await supabase
      ...
      .eq('id', req.params.slug)
      .single();
    if (error || !data) return res.status(404).send(notFoundPage());
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ status: 'ok', supabase: !error ? 'connected' : 'error', tripCount: count || 0 });
  } catch(e) {
    res.json({ status: 'ok', supabase: 'error', error: e.message });
  }
});
...
.empty{display:flex;align-items:center;justify-content:center;min-height:300px;color:rgba(255,255,255,.3);font-size:14px;}
...
function renderTripPage(t) {
  ...
  if (countdown !== null) {
    if (countdown > 0) {
      cdHtml = `<div class="cd-wrap"><div class="cd-sparkles">✨ 🎉 ✨</div><div class="cd-num">${countdown}</div><div class="cd-label">days until your adventure begins!</div><div class="cd-sparkles">🌟 🎊 🌟</div></div>`;
    } else if (countdown === 0) {
      cdHtml = `<div class="cd-wrap cd-today"><div class="cd-sparkles">🎉 🥂 🎉</div><div class="cd-num">Today!</div><div class="cd-label">Your adventure starts now!</div></div>`;
    } else {
      cdHtml = `<div class="cd-wrap cd-active"><div class="cd-sparkles">🌴 ✈️ 🌴</div><div class="cd-num">You're There!</div><div class="cd-label">Enjoy every moment!</div></div>`;
    }
  }

  const galleryHtml = (t.photos && t.photos.length) ? `
  <section class="sec"><div class="sec-label">Photo Gallery</div>
  <div class="swiper"><div class="swiper-track" id="mainTrack">${t.photos.map((p,i)=>`<div class="swipe-slide"><img src="${p}" alt="Trip photo" loading="lazy" onclick="openLB(${i})"></div>`).join('')}</div>
  <div class="swipe-dots" id="mainDots">${t.photos.map((_,i)=>`<div class="dot${i===0?' active':''}" onclick="goSlide(${i})"></div>`).join('')}</div>
  </div></section><hr class="div">` : '';

  const resortBtn = (t.resortPhotos && t.resortPhotos.length) ? `
  <section class="sec" style="text-align:center;"><a href="/trips/${t.id}/resort" class="resort-btn">🏨 View ${t.hotel||'Resort'} Photos <span class="resort-count">${t.resortPhotos.length} photos</span></a></section><hr class="div">` : '';
  ...
  ${t.hotel?`<div class="info-card"><div class="iicon hotel-i">🏨</div><div class="ibody"><div class="ititle">${t.hotel}</div><div class="isub">${t.hotelAddr?t.hotelAddr+'<br>':''}${t.checkin||''}</div></div></div>${resortHeroHtml}`:''}
  ...
  <div class="msg-card"><div class="msg-txt">"${t.message}"</div><div class="msg-from">— ${t.signedFrom||'Voyage Vista Travels'}</div></div></section><hr class="div">` : '';

  const itinerary = (t.days && t.days.some(d=>d.title||(d.events&&d.events.length))) ? `
  <section class="sec"><div class="sec-label">Your Itinerary</div>
  ...
  <div class="day-block">
    ...
    ${(d.events||[]).filter(e=>e.title).map(e=>`<div class="ev-row"><span class="ev-dot ev-dot-${e.type}"></span><div><div class="ev-title">${e.title}${e.time?` <span class="ev-time">${e.time}</span>`:''}</div>${e.flightNum?`<div class="ev-note">Flight ${e.flightNum}${e.route?' · '+e.route:''}</div>`:''}${e.address?`<div class="ev-note">📍 ${e.address}</div>`:''}${e.notes?`<div class="ev-note">${e.notes}</div>`:''}</div></div>`).join('')}
  </div>`).join('')}
  </section><hr class="div">` : '';

  const itineraryDownload = t.itineraryLink ? `
  <section class="sec"><div class="sec-label">Itinerary Document</div>
  ...
  <section class="sec"><div class="sec-label">RSVP / Enquiry</div>
  <div class="book-form">
    <form id="rsvpForm">
...
.hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%,rgba(196,160,87,0.18) 0%,transparent 65%),radial-gradient(ellipse at 80% 80%,rgba(200,100,120,0.1) 0%,transparent 50%);pointer-events:none;}
...
@keyframes cdPulse{0%,100%{text-shadow:0 0 40px rgba(196,160,87,.5)}50%{text-shadow:0 0 80px rgba(232,200,122,.8)}}
.cd-label{font-size:.85rem;color:rgba(255,255,255,.7);margin-top:4px;letter-spacing:.06em;text-transform:uppercase;}
...
.ev-dot-flight{background:#3B6D11;}.ev-dot-hotel{background:#185FA5;}.ev-dot-activity{background:#854F0B;}
.ev-dot-restaurant{background:#993556;}.ev-dot-transport{background:#5F5E5A;}.ev-dot-note{background:#534AB7;}
...
.weather-icon-big{font-size:3rem;}.weather-temp-big{font-size:1.5rem;font-weight:500;color:#1a3550;}
.weather-desc-big{font-size:13px;color:#555;margin-top:3px;}.weather-extras{font-size:12px;color:#888;margin-top:4px;}
...
${galleryHtml}${resortBtn}${logistics}${msgSec}${itinerary}${itineraryDownload}${weatherSec}${packSec}${currSec}${bookSec}${contactSec}
...
<div class="lb" id="lb"><button class="lb-close" onclick="closeLB()">✕</button><button class="lb-prev" onclick="navLB(-1)">‹</button><img class="lb-img" id="lbImg"><button class="lb-next" onclick="navLB(1)">›</button></div>
<script>
var allPhotos=${JSON.stringify(t.photos||[])},lbIdx=0;
var track=document.getElementById('mainTrack'),dotEls=document.querySelectorAll('.dot'),curSlide=0;
function goSlide(i){curSlide=i;if(track)track.style.transform='translateX(-'+i+'00%)';dotEls.forEach(function(d,j){d.classList.toggle('active',j===i);});}
if(track&&allPhotos.length>1){
  var sx=0;
  track.addEventListener('touchstart',function(e){sx=e.touches[0].clientX;},{passive:true});
  track.addEventListener('touchend',function(e){var dx=e.changedTouches[0].clientX-sx;if(dx<-40)goSlide(Math.min(curSlide+1,allPhotos.length-1));else if(dx>40)goSlide(Math.max(curSlide-1,0));});
}
function openLB(i){lbIdx=i;document.getElementById('lbImg').src=allPhotos[i];document.getElementById('lb').classList.add('open');}
function closeLB(){document.getElementById('lb').classList.remove('open');}
...
if(form){form.addEventListener('submit',function(e){e.preventDefault();fetch('/api/booking',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:document.getElementById('bn').value,email:document.getElementById('be').value,trip:'${t.id}',to:'Hello@voyagevista.ca'})});form.style.display='none';document.getElementById('bsuccess').style.display='block';});}
</script>
</body></html>`;
}
