...
const TRIPS_FILE = path.join(__dirname, 'trips-data.json');
...
function generateSlug(guestName, occasion) {
  return (guestName + '-' + occasion).toLowerCase()
    .replace(/[^a-z0-9\s-]/g,'').replace(/\s+/g,'-').replace(/-+/g,'-').substring(0,60);
}

app.get('/api/trips', (req, res) => {
  const data = loadTrips();
  const list = Object.values(data.trips).map(t => ({
...
app.get('/trips/admin', (req, res) => res.sendFile(path.join(__dirname, 'trips', 'admin.html')));
app.get('/trips', (req, res) => res.redirect('/trips/admin'));
...
function renderTripPage(t) {
  ...
  <div class="photo-grid">${t.photos.map(p=>`<div class="photo-item"><img src="${p}" alt="Trip photo" loading="lazy"></div>`).join('')}</div></section><hr class="div">` : '';
  ...
  ${t.hotel?`<div class="info-card"><div class="iicon hotel-i">🏨</div><div class="ibody"><div class="ititle">${t.hotel}</div><div class="isub">${t.hotelAddr?t.hotelAddr+'<br>':''}${t.checkin||''}</div></div></div>`:''}
  </section><hr class="div">` : '';

  const itinerary = (t.days && t.days.some(d=>d.title)) ? `
  <section class="sec"><div class="sec-label">Your Itinerary</div>
  ${t.days.filter(d=>d.title).map(d=>`<div class="day-block"><div class="day-lbl">${d.label}</div><div class="day-ttl">${d.title}</div>${d.note?`<div class="day-note">${d.note}</div>`:''}</div>`).join('')}
  </section><hr class="div">` : '';

  const pdfSec = t.pdfItinerary ? `
  <section class="sec"><div class="sec-label">Download Itinerary</div>
  <a href="${t.pdfItinerary}" download="itinerary-${t.id}.pdf" class="dl-btn">📄 Download Full Itinerary PDF</a>
  </section><hr class="div">` : '';
  ...
  <div class="curr-card">
    ...
    ${t.currency.exchangeRate?`<div class="curr-row"><span class="curr-lbl">Approx. Rate</span><span class="curr-val">${t.currency.exchangeRate}</span></div>`:''}
    ${t.currency.dailyBudget?`<div class="curr-row"><span class="curr-lbl">Suggested Daily Budget</span><span class="curr-val">${t.currency.dailyBudget}</span></div>`:''}
  ...
  <div class="book-form">
    <form onsubmit="submitB(event)">
      ...
      <textarea placeholder="Special requests, dietary needs, or any questions..." rows="3" id="bn2"></textarea>
      <button type="submit" class="sub-btn">Send RSVP to Voyage Vista ✈</button>
    </form>
    <div id="bsuccess" style="display:none;text-align:center;padding:24px;color:#7ec98f;font-size:15px;">✅ Thank you! We'll be in touch within 1–2 business days.</div>
  </div></section><hr class="div">` : '';

  const contactSec = t.advisorPhone ? `
  <section class="sec" style="text-align:center;">
  <div class="adv-card"><div class="adv-lbl">Your travel advisor is available 24/7</div>
  <a href="tel:${(t.advisorPhone||'').replace(/\D/g,'')}" class="adv-phone">${t.advisorPhone}</a>
  <div style="margin-top:6px;"><a href="mailto:hello@voyagevista.ca" style="color:rgba(255,255,255,0.4);font-size:13px;text-decoration:none;">hello@voyagevista.ca</a></div>
  </div></section>` : '';
...
.h-dest{font-size:1.05rem;color:rgba(255,255,255,.6);margin-bottom:28px;letter-spacing:.04em;}
...
.day-ttl{font-size:16px;font-weight:500;}.day-note{font-size:13px;color:#666;margin-top:5px;line-height:1.6;}
...
.book-form{background:#fff;border-radius:14px;padding:28px;border:1px solid rgba(196,160,87,.1);box-shadow:0 2px 16px rgba(0,0,0,.07);}
...
function submitB(e){
  e.preventDefault();
  document.querySelector('.book-form form').style.display='none';
